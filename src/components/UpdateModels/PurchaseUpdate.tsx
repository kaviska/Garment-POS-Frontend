...
interface FormData {
  ...
  variation_id: number | null | string | undefined;
  variation_option_id: number | null | string | undefined;
  image: File | null | string; // Add image field
}
...
export default function PurchaseUpdate({
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    image: initialData?.image || null, // Add image field
  });
  ...
  const handleUpdate = async () => {
    try {
      ...
      if (formData.variation_id) {
        formDataToSend.append("variation_id", String(formData.variation_id));
      }
      if (formData.variation_option_id) {
        formDataToSend.append("variation_option_id", String(formData.variation_option_id));
      }

      // Handle image - if it's a File object, append it; if it's a string (existing image), don't append
          ...
          <div className="grid grid-cols-1 gap-8">
            {inputFields.map((field) => (
              <FormGenerator
                ...
                previewUpadte={field.name === "image" && typeof formData.image === "string" ? formData.image : undefined}
              />
            ))}
